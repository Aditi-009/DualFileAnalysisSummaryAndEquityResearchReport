...
class EnhancedDualFileAnalysisBot:
    ...
    def create_comprehensive_charts(self, news_df: pd.DataFrame, reddit_df: pd.DataFrame, 
        ...
        try:
            ...
            # Chart 1: Data Volume Comparison
            chart_paths.append(self._create_volume_comparison_chart(data_analysis))
            
            # Chart 2: Text Length Distribution
            chart_paths.append(self._create_text_length_chart(news_df, reddit_df))
            ...
            # Chart 8: Sentiment Timeline (if date data available)
            chart_paths.append(self._create_timeline_chart(news_df, reddit_df, data_analysis))
    ...
    def _create_volume_comparison_chart(self, data_analysis: Dict[str, Any]) -> str:
        """Create data volume comparison chart with better error handling"""
        temp_path = ""
        try:
            ...
            volumes = [
                data_analysis.get('news_analysis', {}).get('total_articles', 0),
                data_analysis.get('reddit_analysis', {}).get('total_posts', 0)
            ]
            
            # Handle case where both volumes are 0
            if sum(volumes) == 0:
                volumes = [1, 1]  # Placeholder to avoid empty chart
                categories = ['No News Data', 'No Social Data']
            ...
            logger.error(f"Error creating volume comparison chart: {str(e)}")
            if plt.get_fignums():  # Check if figure exists
                plt.close('all')
            # Clean up temp file if it was created but chart failed
    ...
    def _create_text_length_chart(self, news_df: pd.DataFrame, reddit_df: pd.DataFrame) -> str:
        """Create text length distribution chart"""
        try:
            fig, axes = plt.subplots(2, 2, figsize=(15, 10))
            
            # News text length histogram
            if news_df is not None and not news_df.empty:
                news_text_col = self.identify_text_column(news_df)
                if news_text_col:
                    news_lengths = news_df[news_text_col].astype(str).str.len()
                    axes[0, 0].hist(news_lengths, bins=30, color='#1f4e79', alpha=0.7, edgecolor='black')
                    axes[0, 0].set_title('News Article Length Distribution', fontweight='bold')
                    axes[0, 0].set_xlabel('Character Count')
                    axes[0, 0].set_ylabel('Frequency')
                    axes[0, 0].grid(True, alpha=0.3)
            
            # Reddit text length histogram
            if reddit_df is not None and not reddit_df.empty:
                reddit_text_col = self.identify_text_column(reddit_df)
                if reddit_text_col:
                    reddit_lengths = reddit_df[reddit_text_col].astype(str).str.len()
                    axes[0, 1].hist(reddit_lengths, bins=30, color='#2E8B57', alpha=0.7, edgecolor='black')
                    axes[0, 1].set_title('Social Media Post Length Distribution', fontweight='bold')
            ...
            if news_df is not None and not news_df.empty:
                news_text_col = self.identify_text_column(news_df)
                if news_text_col:
                    lengths_data.append(news_df[news_text_col].astype(str).str.len())
                    labels.append('News')
            
            if reddit_df is not None and not reddit_df.empty:
                reddit_text_col = self.identify_text_column(reddit_df)
                if reddit_text_col:
                    lengths_data.append(reddit_df[reddit_text_col].astype(str).str.len())
                    labels.append('Social Media')
            
            if lengths_data:
                axes[1, 0].boxplot(lengths_data, labels=labels)
                axes[1, 0].set_title('Text Length Comparison (Box Plot)', fontweight='bold')
            ...
            if lengths_data:
                stats_data = []
                for i, data in enumerate(lengths_data):
                    stats_data.append([
                        labels[i],
                        f"{data.mean():.0f}",
                        f"{data.median():.0f}",
                ...
                table_data = [['Source', 'Mean', 'Median', 'Std Dev', 'Min', 'Max']] + stats_data
                table = axes[1, 1].table(cellText=table_data, loc='center', cellLoc='center')
                table.auto_set_font_size(False)
                table.set_fontsize(9)
                table.scale(1, 2)
                axes[1, 1].set_title('Text Length Statistics', fontweight='bold', pad=20)
            ...
            logger.error(f"Error creating text length chart: {str(e)}")
            plt.close()
            return ""
    ...
    def _create_timeline_chart(self, news_df: pd.DataFrame, reddit_df: pd.DataFrame, 
                                data_analysis: Dict[str, Any]) -> str:
        """Create timeline analysis chart"""
        try:
            fig, axes = plt.subplots(2, 2, figsize=(15, 10))
            
            # Try to find date columns and create timeline
            news_timeline_created = False
            reddit_timeline_created = False
            
            # News timeline
            if news_df is not None and not news_df.empty:
                date_cols = [col for col in news_df.columns if 'date' in col.lower() or 'time' in col.lower()]
                for col in date_cols:
                    try:
                        news_df_copy = news_df.copy()
                        news_df_copy[col] = pd.to_datetime(news_df_copy[col])
                        daily_counts = news_df_copy.groupby(news_df_copy[col].dt.date).size()
                        
                        axes[0, 0].plot(daily_counts.index, daily_counts.values, marker='o', 
                                        linewidth=2, color='#1f4e79', markersize=6)
                        axes[0, 0].set_title('News Articles Timeline', fontweight='bold')
                        axes[0, 0].set_xlabel('Date')
                        axes[0, 0].set_ylabel('Number of Articles')
                        axes[0, 0].grid(True, alpha=0.3)
                        axes[0, 0].tick_params(axis='x', rotation=45)
                        news_timeline_created = True
                        break
                    except:
                        continue
            
            if not news_timeline_created:
                axes[0, 0].text(0.5, 0.5, 'No Date Information\nAvailable for News', 
            ...
            # Reddit timeline
            if reddit_df is not None and not reddit_df.empty:
                date_cols = [col for col in reddit_df.columns if 'date' in col.lower() or 'time' in col.lower()]
                for col in date_cols:
                    try:
                        reddit_df_copy = reddit_df.copy()
                        reddit_df_copy[col] = pd.to_datetime(reddit_df_copy[col])
                        daily_counts = reddit_df_copy.groupby(reddit_df_copy[col].dt.date).size()
                        
                        axes[0, 1].plot(daily_counts.index, daily_counts.values, marker='s', 
                                        linewidth=2, color='#2E8B57', markersize=6)
                        axes[0, 1].set_title('Social Media Posts Timeline', fontweight='bold')
                        axes[0, 1].set_xlabel('Date')
                        axes[0, 1].set_ylabel('Number of Posts')
                        axes[0, 1].grid(True, alpha=0.3)
                        axes[0, 1].tick_params(axis='x', rotation=45)
                        reddit_timeline_created = True
                        break
                    except:
                        continue
            
            if not reddit_timeline_created:
                axes[0, 1].text(0.5, 0.5, 'No Date Information\nAvailable for Social Media', 
                                ha='center', va='center', transform=axes[0, 1].transAxes, 
                                fontsize=12, fontweight='bold')
                axes[0, 1].set_title('Social Media Posts Timeline', fontweight='bold')
            
            # Activity heatmap (day of week analysis)
            if news_timeline_created or reddit_timeline_created:
                days = ['Mon', 'Tue', 'Wed', 'Thu', 'Fri', 'Sat', 'Sun']
                hours = list(range(24))
                
                # Create a mock activity heatmap
                activity_data = np.random.rand(7, 24) * 100
                
                im = axes[1, 0].imshow(activity_data, cmap='YlOrRd', aspect='auto')
                axes[1, 0].set_title('Activity Heatmap (Simulated)', fontweight='bold')
                ...
                axes[1, 0].text(0.5, 0.5, 'Timeline Analysis\nNot Available', 
                                ha='center', va='center', transform=axes[1, 0].transAxes, 
                                fontsize=12, fontweight='bold')
                axes[1, 0].set_title('Activity Heatmap', fontweight='bold')
            
            # Summary statistics
            axes[1, 1].axis('off')
            timeline_summary = [['Timeline Metric', 'Value']]
            
            if news_df is not None and reddit_df is not None:
                total_data_points = len(news_df) + len(reddit_df)
                timeline_summary.extend([
                    ['Total Data Points', f"{total_data_points:,}"],
                    ['News Articles', f"{len(news_df):,}"],
                    ['Social Media Posts', f"{len(reddit_df):,}"],
                    ['Date Data Available', 'Yes' if (news_timeline_created or reddit_timeline_created) else 'No'],
                    ['Analysis Period', 'Last 30 days (estimated)']
                ])
            
            table = axes[1, 1].table(cellText=timeline_summary, loc='center', cellLoc='center')
            table.auto_set_font_size(False)
            table.set_fontsize(10)
            table.scale(1, 2)
            # Color header row
            for i in range(len(timeline_summary[0])):
                table[(0, i)].set_facecolor('#2E8B57')
                table[(0, i)].set_text_props(weight='bold', color='white')
            axes[1, 1].set_title('Timeline Summary', fontweight='bold', pad=20)
    ...
    def create_enhanced_comprehensive_charts(self, news_df: pd.DataFrame, reddit_df: pd.DataFrame, 
        ...
        try:
            ...
            try:
                correlation_chart = self._create_correlation_analysis_chart(news_df, reddit_df, stock_data)
                if correlation_chart:
                    enhanced_charts.append(correlation_chart)
                    logger.info("Created correlation analysis chart")
            except Exception as e:
                logger.warning(f"Could not create correlation analysis chart: {e}")
            
            # 2. Advanced Sentiment Timeline
            try:
                sentiment_timeline = self._create_advanced_sentiment_timeline(news_df, reddit_df, sentiment_data)
                if sentiment_timeline:
                    enhanced_charts.append(sentiment_timeline)
                    logger.info("Created advanced sentiment timeline")
            except Exception as e:
    ...
    def create_advanced_pdf_with_enhanced_charts(self, company_info: Dict[str, str], stock_data: Dict[str, Any], 
        ...
        try:
            ...
            chart_descriptions = {
                0: "Data Volume and Quality Analysis - Overview of data sources and completeness",
                1: "Sentiment Analysis - Market sentiment from news and social media sources", 
                2: "Stock Price and Technical Indicators - Price trends and moving averages",
                3: "Risk Assessment - Volatility analysis and risk categorization",
                4: "Correlation Analysis - Relationships between sentiment, volume, and price movements",
                5: "Advanced Sentiment Timeline - Temporal sentiment patterns with volatility analysis",
                6: "Market Comparison - Performance benchmarking against market indices",
                7: "Technical Analysis - RSI, MACD, and Bollinger Bands analysis",
                8: "News Impact Analysis - News volume correlation with price movements",
                9: "Social Media Engagement - Engagement patterns and sentiment correlation",
                
                10: "Comprehensive Dashboard - Executive summary of all key metrics"
            }
